...
void loop() {
  ...
  if (p.z > MINPRESSURE && p.z < MAXPRESSURE) {
    ...
    if (menuButton.justReleased() && menuOpen == false) {
      ...
      tft.println("Welcome to Fruity Fun! The goal is to match 3 or more fruits in a row or in a column. Select two adjacent fruits to switch them.");
      tft.println("In our world, gravity works slightly differently. The down arrow indicates which way fruits will fall. -->");

      tft.setCursor(0, 200);
      tft.setTextColor(WHITE);  tft.setTextSize(1);
      tft.println(" SE101 2017 Developers: Beini Fang, Ellen Huang, Lindsey Jin");
