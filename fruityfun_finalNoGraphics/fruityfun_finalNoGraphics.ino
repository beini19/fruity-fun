...
bool menuOpen = false;  //Value will be true when menu screen is displayed
...
void gridFlash(int x1, int y1, int x2, int y2) {
  for (int i = 0; i < 3; i++) {
    highlightGrid(x1, y1, RED);
    highlightGrid(x2, y2, RED);
    delay(100);
...
int displayMatch(int matchedElement) {
  if (matchedElement == 0) return -6;
  else return -1 * matchedElement;
}

void countPoints(int matched[ROWS][COLUMNS], int countMatches[6]) {
  for (int x = 0; x < ROWS; x++) {
    for (int y = 0; y < COLUMNS; y++) {
      if (matched[x][y] <= -1 && matched[x][y] >= -6) {
        countMatches[-1 * matched[x][y]]++;
      }
    }
  }
  for (int i = 0; i < 6; i++) {
    ...
    //Display new score
    tft.fillRect(159, 289, 100, 40, BLACK); //Draw black rectangle over original score. Parameters: (x_corner, y_corner, length, width, colour)
...
bool checkGrid (int grid[ROWS][COLUMNS], int matched[ROWS][COLUMNS]) {
  bool change = false;
  int match = 0, r = -1;
...
void setup() {
  ...
  //Draw menu button
  menuButton.initButton(&tft, BUTTON_X, BUTTON_Y, BUTTON_W, BUTTON_H, WHITE, BLUE, WHITE, "Menu", BUTTON_TEXTSIZE);  // x, y, w, h, outline, fill, text
  menuButton.drawButton();
  
  //Draw reset button
  resetButton.initButton(&tft, BUTTON_X + BUTTON_W + BUTTON_SPACING_X, BUTTON_Y, BUTTON_W, BUTTON_H, WHITE, BLUE, WHITE, "Reset", BUTTON_TEXTSIZE);  // x, y, w, h, outline, fill, text
  ...
  reset(matched);   //set everything in matched to 8
  reset(posMove);   //set everything in posMove to 8
...
void loop() {
  ...
  if (p.z > MINPRESSURE && p.z < MAXPRESSURE) {
    ...
    if (menuButton.justReleased() && menuOpen == false) {
      ...
      tft.println("Welcome to Fruity Fun! The goal is to");
      tft.println("match 3 or more fruits in a row or in");
      tft.println("a column. Select two adjacent fruits to");
      tft.println("switch them.");
      tft.println();
      tft.println();
      tft.println("In our world, gravity works slightly");
      tft.println("differently. The down arrow indicates");
      tft.println("which way fruits will fall.");
      ...
      tft.setCursor(0, 280);
      tft.setTextColor(WHITE);  tft.setTextSize(1);
      tft.println("SE101 2017");
      tft.println("Developers:");
      tft.println("Beini Fang, Ellen Huang, Lindsey Jin");
